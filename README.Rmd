...
load(file = "data/census_divisions_2016_carto_df.rda")
load(file = "data/census_divisions_2016_noterr_carto_df.rda")
```
...
```{r echo = FALSE}
ggprovincecatro <- ggplot(provinces_territories_carto_df, aes(long, lat, group = group, fill = population)) +
  geom_polygon() +
  theme_light() +
...
ggprovincenoterrcatro <- ggplot(provinces_noterr_carto_df, aes(long, lat, group = group, fill = population)) +
  geom_polygon() +
  theme_light() +
  scale_fill_gradient(low = "darkslategray", high = "darkslategray3") + 
  theme(panel.grid = element_blank(),
