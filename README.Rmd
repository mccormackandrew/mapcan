...
```{r echo = FALSE}
# Obtain geographic data with mapcan() function
canada_ridings <- mapcan(boundaries = ridings,
       type = standard)

canada_ridings <- left_join(canada_ridings, fed2015, by = c("riding_code" = "riding_code"))

ggplot(canada_ridings, aes(long, lat, group = group, fill = party)) +
  geom_polygon() +
